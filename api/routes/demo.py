...
**These are destructive and gated.** `AFI_DEMO_MODE` defaults to on because the
V1 deployment is a single local profile with no authentication and no network
exposure (ADR-014) — but the moment either of those changes, this is the first
thing that must be turned off, so it is a switch rather than an assumption.
"""

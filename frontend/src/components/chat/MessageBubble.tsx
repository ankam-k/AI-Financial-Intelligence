      ...
      {turn.status === 'pending' ? (
        ...
        <div className="turn__answer">
          ...
          {response.validation_failures.length > 0 ? (
            <details className="turn__meta">
              <summary style={{ cursor: 'pointer' }}>
                A generated version was rejected ({response.validation_failures.length} check
                {response.validation_failures.length === 1 ? '' : 's'})
              </summary>
              <ul style={{ margin: '6px 0 0', paddingLeft: 18 }}>
                {response.validation_failures.map((failure, index) => (
                  <li key={index}>

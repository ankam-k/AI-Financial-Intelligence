...
def behaviour_relationships(
    ...
    # G4 — effect size, before G5, so a trivial difference does not consume an
    # FDR slot and make a real one harder to detect.
    ...
    # G5 — Benjamini-Hochberg across the surviving hypotheses.
    q_values = benjamini_hochberg(
        [c.result.p_value for c in survivors], q=gates.fdr_q
    )

    accepted: list[tuple[_Candidate, float]] = []
    for candidate, q_value in zip(survivors, q_values):
